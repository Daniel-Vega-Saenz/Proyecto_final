...
def pausa():
    global pausar#Pone pausa a la cancion independientemente de la posicion en la que este
    if pausar:# consicionamos la funcion de pausa para definir en que momento y en qeu condiciones se tiene que pausar, Evitando que se pause sola
        pygame.mixer.music.unpause()#Usamos la funcion unpause para  determinar si la canicion esta pausada o no y por lo tanto que no se active la funcion
        pausar=False
    else:
        pygame.mixer.music.pause()#Al estar la cancion en pausa, le decimos que el boton estara activo y que al seleccioanral se desactivara la funcion 
        pausar=True


pantalla=Listbox(bg="#FF5733", fg="blue",width=60,selectbackground="white", selectforeground="black")#ABRE LA PANTALLA
